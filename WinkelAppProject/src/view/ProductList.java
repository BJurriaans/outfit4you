...
public class ProductList extends JPanel implements MouseListener, ActionListener {
    ...
    private void addProductItems() {
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            lblProduct.setName(String.valueOf(product.getProductId()));  //geef dit label het product ID als naam, is nodig om verder te gaan bij ProductDetails.java
            ...
            btnOrder.addActionListener(this); //zorgt ervoor dat een product aan basket toegevoegd kan worden
            add(btnOrder);
        }
    ...
    public void mouseClicked(MouseEvent event) {
        ...
        if (label.getName().equals("-1")) {
            ...
            WinkelApplication.getInstance().showPanel(new view.ProductDetails(Integer.parseInt(label.getName()))); //alles wat niet op title geklikt is, heeft z'n eigen ID, en gaat naar de product details van dat product (zie comment regel bij method addProductItems)
        }
    }
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        WinkelApplication.getBasket().addProduct(product); //voegt product toe aan basket
    }
